...
const searchJobs = async (role) => {
  ...
  try {
    ...
    console.log(`Navigating to Naukri to search for: ${role}`);
    await page.goto('https://www.naukri.com/', { waitUntil: 'domcontentloaded', timeout: 15000 });
    
    await page.waitForTimeout(3000);
    const searchInputs = await page.$$('input[placeholder*="skills"], input.suggestor-input');
    if (searchInputs.length > 0) {
      await searchInputs[0].fill(role || 'Software Developer');
      await page.keyboard.press('Enter');
      await page.waitForTimeout(5000);
    }
    
    const jobs = await page.evaluate(() => {
      const elements = Array.from(document.querySelectorAll('.jobTuple, .srp-jobtuple-wrapper, article'));
      const results = [];
      ...
      for (const el of elements) {
        ...
        const companyEl = el.querySelector('.companyInfo, .subTitle');
        const locationEl = el.querySelector('.location, .locWdth');
        const salaryEl = el.querySelector('.salary, .sal');
        
        const title = titleEl ? titleEl.innerText.trim() : el.innerText.split('\n')[0];
        const company = companyEl ? companyEl.innerText.trim() : 'Naukri Partner';
        let link = el.tagName === 'A' ? el.href : '';
        if (!link) {
           const a = el.querySelector('a.title, a');
           if (a) link = a.href;
        }
